...
if TYPE_CHECKING:
    ...
    from ._well_data import WellData, WellDataApply
...
class Well(DomainModel):
    """This represents the reading version of well.
    ...
    def as_apply(self) -> WellApply:
        """Convert this read version of well to the writing version."""
        return WellApply(
            space=self.space,
...
class WellApply(DomainModelApply):
    """This represents the writing version of well.
    ...
    Args:
        ...
        tags: The tag field.
        version_: The version field.
        existing_version: Fail the ingestion request if the well version is greater than or equal to this value.
    ...
    def _to_instances_apply(
        ...
        write_view = (view_by_write_class and view_by_write_class.get(type(self))) or dm.ViewId(
            "IntegrationTestsImmutable", "Well", "952d7e55cdf2cc"
        )
...
class WellList(DomainModelList[Well]):
    """List of wells in the read version."""

    _INSTANCE = Well

    def as_apply(self) -> WellApplyList:
        """Convert these read versions of well to the writing versions."""
        return WellApplyList([node.as_apply() for node in self.data])


class WellApplyList(DomainModelApplyList[WellApply]):
    """List of wells in the writing version."""

    _INSTANCE = WellApply


def _create_well_filter(
    view_id: dm.ViewId,
