...
class ProjectedBottomHoleLocation(DomainModel):
    """This represents the reading version of projected bottom hole location.
    ...
    def as_apply(self) -> ProjectedBottomHoleLocationApply:
        """Convert this read version of projected bottom hole location to the writing version."""
...
class ProjectedBottomHoleLocationApply(DomainModelApply):
    """This represents the writing version of projected bottom hole location.
    ...
    Args:
        ...
        spatial_parameter_type_id: The spatial parameter type id field.
        wgs_84_coordinates: The wgs 84 coordinate field.
        existing_version: Fail the ingestion request if the projected bottom hole location version is greater than or equal to this value.
...
class ProjectedBottomHoleLocationList(DomainModelList[ProjectedBottomHoleLocation]):
    """List of projected bottom hole locations in the read version."""

    _INSTANCE = ProjectedBottomHoleLocation

    def as_apply(self) -> ProjectedBottomHoleLocationApplyList:
        """Convert these read versions of projected bottom hole location to the writing versions."""
        return ProjectedBottomHoleLocationApplyList([node.as_apply() for node in self.data])


class ProjectedBottomHoleLocationApplyList(DomainModelApplyList[ProjectedBottomHoleLocationApply]):
    """List of projected bottom hole locations in the writing version."""

    _INSTANCE = ProjectedBottomHoleLocationApply


def _create_projected_bottom_hole_location_filter(
    view_id: dm.ViewId,
