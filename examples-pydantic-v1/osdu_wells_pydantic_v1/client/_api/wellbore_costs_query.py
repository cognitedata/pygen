from __future__ import annotations

from typing import TYPE_CHECKING
from cognite.client import data_modeling as dm
from ._core import DEFAULT_QUERY_LIMIT, QueryStep, QueryAPI, T_DomainModelList
from osdu_wells_pydantic_v1.client.data_classes import (
    WellboreCosts,
    WellboreCostsApply,
)
from osdu_wells_pydantic_v1.client.data_classes._wellbore_costs import (
    _WELLBORECOSTS_PROPERTIES_BY_FIELD,
)


class WellboreCostsQueryAPI(QueryAPI[T_DomainModelList]):
    def query(
        self,
        retrieve_wellbore_cost: bool = True,
    ) -> T_DomainModelList:
        """Execute query and return the result.

        Args:
            retrieve_wellbore_cost: Whether to retrieve the wellbore cost or not.

        Returns:
            The list of the source nodes of the query.

        """
        from_ = self._builder[-1].name
        if retrieve_wellbore_cost and not self._builder[-1].name.startswith("wellbore_cost"):
            self._builder.append(
                QueryStep(
                    name=self._builder.next_name("wellbore_cost"),
                    expression=dm.query.NodeResultSetExpression(
                        filter=None,
                        from_=from_,
                    ),
                    select=dm.query.Select(
                        [
                            dm.query.SourceSelector(
                                self._view_by_write_class[WellboreCostsApply],
                                list(_WELLBORECOSTS_PROPERTIES_BY_FIELD.values()),
                            )
                        ]
                    ),
                    result_cls=WellboreCosts,
                    max_retrieve_limit=-1,
                ),
            )

        return self._query()
