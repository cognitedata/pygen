...
from osdu_wells_pydantic_v1.client.data_classes._wellbore import _WELLBORE_PROPERTIES_BY_FIELD


class WellboreMetaAPI:
...
class WellboreAPI(TypeAPI[Wellbore, WellboreApply, WellboreList]):
    def __init__(self, client: CogniteClient, view_by_write_class: dict[type[DomainModelApply], dm.ViewId]):
        ...
        self.meta = WellboreMetaAPI(client)

    def apply(
        self, wellbore: WellboreApply | Sequence[WellboreApply], replace: bool = False
    ) -> dm.InstancesApplyResult:
        """Add or update (upsert) wellbores.

        Note: This method iterates through all nodes linked to wellbore and create them including the edges
        ...
        Examples:
            ...
            Create a new wellbore:
                ...
                >>> from osdu_wells_pydantic_v1.client import OSDUClient
                >>> from osdu_wells_pydantic_v1.client.data_classes import WellboreApply
                >>> client = OSDUClient()
                >>> wellbore = WellboreApply(external_id="my_wellbore", ...)
                >>> result = client.wellbore.apply(wellbore)

        """
        if isinstance(wellbore, WellboreApply):
            instances = wellbore.to_instances_apply(self._view_by_write_class)
        else:
            instances = WellboreApplyList(wellbore).to_instances_apply(self._view_by_write_class)
    ...
    @staticmethod
    def _set_meta(wellbores: Sequence[Wellbore], meta_edges: Sequence[dm.Edge]):
        edges_by_start_node: Dict[Tuple, List] = defaultdict(list)
        for edge in meta_edges:
            edges_by_start_node[edge.start_node.as_tuple()].append(edge)

        for wellbore in wellbores:
            node_id = wellbore.id_tuple()
            if node_id in edges_by_start_node:
                wellbore.meta = [edge.end_node.external_id for edge in edges_by_start_node[node_id]]
