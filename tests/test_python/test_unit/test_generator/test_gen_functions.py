...
from tests.test_python.constants import EXAMPLES, SDK_NAME_PYTHON, SDK_NAME_TYPESCRIPT
...
@pytest.fixture(scope="session")
def actual_typescript_sdk_example_model() -> dict[Path, str]:
    with monkeypatch_pygen_client() as mocked_client:
        mocked_client.data_models.retrieve.return_value = [create_example_data_model_response()]
        sdk_config = PygenSDKConfig(
            top_level_package=SDK_NAME_TYPESCRIPT,
...
@pytest.mark.parametrize("file_path", load_example_model(SDK_NAME_TYPESCRIPT).keys(), ids=lambda p: p.as_posix())
def test_generate_typescript_sdk_example_model(
    file_path: Path, actual_typescript_sdk_example_model: dict[Path, str]
) -> None:
    expected_filepath = EXAMPLES / SDK_NAME_TYPESCRIPT / file_path
    if not expected_filepath.exists():
        pytest.fail(f"Expected file {file_path} not found in {EXAMPLES.as_posix()!r} directory.")
    expected_content = expected_filepath.read_text(encoding="utf-8")

    actual_content = actual_typescript_sdk_example_model.get(file_path)
    if actual_content is None:
        pytest.fail(f"Generated SDK is missing expected file: {file_path}")
