...
def handle(data: dict, client: CogniteClient) -> dict:
    try:
        ...
        if last_entry_this_file:
            suffix = (
                f" in function {last_entry_this_file.name} on"
                f" line {last_entry_this_file.lineno}: {last_entry_this_file.line}"
            )
...
def to_direct_relations_by_source_by_node(
    ...
    for annotation in annotations:
        ...
        if mapping.start_node_view.direct_relation_property is not None:
            ...
            raise ValueError(
                f"Neither file source nor entity source "
                f"has a direct relation property for annotation {annotation.external_id}"
            )
