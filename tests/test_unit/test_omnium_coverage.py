"""
The goal of the model Omnium is to cover all properties types of a view.
The test in this file is to ensure that the model Omnium is up-to-date with the
    ...
    for view in views:
        for prop in view.properties.values():
            if isinstance(prop, dm.MappedProperty):
                properties.add(
                    (prop.type._type, isinstance(prop.type, ListablePropertyType) and prop.type.is_list, prop.nullable)
                )
    return properties
...
def existing_mapped_properties() -> set[tuple[str, bool, bool]]:
    ...
    return {
        (t._type, listable and issubclass(t, ListablePropertyType), nullable)
        for t, listable, nullable in product(types, [True, False], [True, False])
    } - NOT_SUPPORTED
