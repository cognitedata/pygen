...
class CogProcessQueryAPI(QueryAPI[T_DomainModelList]):
    def __init__(
        ...
        self._builder.append(
            QueryStep(
                name=self._builder.next_name("cog_proces"),
                expression=dm.query.NodeResultSetExpression(
                    from_=self._builder[-1].name if self._builder else None,
                    filter=filter_,
                ),
                select=dm.query.Select([dm.query.SourceSelector(self._view_by_write_class[CogProcessApply], ["*"])]),
    ...
    def query(
        ...
        Args:
            retrieve_bid: Whether to retrieve the bid for each cog proces or not.
            retrieve_date_transformations: Whether to retrieve the date transformation for each cog proces or not.
            retrieve_transformation: Whether to retrieve the transformation for each cog proces or not.
