...
class DependentOnNonWritableAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [DependentOnNonWritable.model_validate(item) for item in unpacked]
        else:
            retrieved = [DependentOnNonWritable.model_construct(**item) for item in unpacked]
        return DependentOnNonWritableList(retrieved)
