...
class Implementation1NonWriteableAPI(NodeReadAPI[Implementation1NonWriteable, Implementation1NonWriteableList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [Implementation1NonWriteable.model_validate(item) for item in unpacked]
        else:
            retrieved = [Implementation1NonWriteable.model_construct(**item) for item in unpacked]
        return Implementation1NonWriteableList(retrieved)
