...
class ConnectionItemGAPI(NodeAPI[ConnectionItemG, ConnectionItemGWrite, ConnectionItemGList, ConnectionItemGWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [ConnectionItemG.model_validate(item) for item in unpacked]
        else:
            retrieved = [ConnectionItemG.model_construct(**item) for item in unpacked]
        return ConnectionItemGList(retrieved)
