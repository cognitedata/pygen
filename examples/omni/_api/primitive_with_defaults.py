...
class PrimitiveWithDefaultsAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [PrimitiveWithDefaults.model_validate(item) for item in unpacked]
        else:
            retrieved = [PrimitiveWithDefaults.model_construct(**item) for item in unpacked]
        return PrimitiveWithDefaultsList(retrieved)
