...
class PrimitiveRequiredListedAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [PrimitiveRequiredListed.model_validate(item) for item in unpacked]
        else:
            retrieved = [PrimitiveRequiredListed.model_construct(**item) for item in unpacked]
        return PrimitiveRequiredListedList(retrieved)
