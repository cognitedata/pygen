...
class ConnectionItemEAPI(NodeAPI[ConnectionItemE, ConnectionItemEWrite, ConnectionItemEList, ConnectionItemEWriteList]):
    ...
    def apply(
        ...
        Note: This method iterates through all nodes and timeseries linked to connection_item_e
        and creates them including the edges
        between the nodes. For example, if any of
        `inwards_single` or `inwards_single_property`
    ...
    def list(
        ...
        Args:
            ...
            sort: (Advanced) If sort_by and direction are not sufficient, you can write your own sorting.
                ...
                specify the direction for each field as well as how to handle null values.
            retrieve_connections: Whether to retrieve `direct_reverse_multi`, `direct_reverse_single`, `inwards_single` and `inwards_single_property`
                for the connection item es. Defaults to 'skip'.
