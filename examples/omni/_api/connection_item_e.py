...
class ConnectionItemEAPI(NodeAPI[ConnectionItemE, ConnectionItemEWrite, ConnectionItemEList, ConnectionItemEWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [ConnectionItemE.model_validate(item) for item in unpacked]
        else:
            retrieved = [ConnectionItemE.model_construct(**item) for item in unpacked]
        return ConnectionItemEList(retrieved)
