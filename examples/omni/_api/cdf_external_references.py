...
class CDFExternalReferencesAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CDFExternalReferences.model_validate(item) for item in unpacked]
        else:
            retrieved = [CDFExternalReferences.model_construct(**item) for item in unpacked]
        return CDFExternalReferencesList(retrieved)
