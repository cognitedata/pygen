...
class ConnectionItemFAPI(NodeAPI[ConnectionItemF, ConnectionItemFWrite, ConnectionItemFList, ConnectionItemFWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [ConnectionItemF.model_validate(item) for item in unpacked]
        else:
            retrieved = [ConnectionItemF.model_construct(**item) for item in unpacked]
        return ConnectionItemFList(retrieved)
