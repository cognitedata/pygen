...
class CDFExternalReferencesListedAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CDFExternalReferencesListed.model_validate(item) for item in unpacked]
        else:
            retrieved = [CDFExternalReferencesListed.model_construct(**item) for item in unpacked]
        return CDFExternalReferencesListedList(retrieved)
