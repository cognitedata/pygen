...
class NodeReadAPI(Generic[T_DomainModel, T_DomainModelList], ABC):
    ...
    def _retrieve(
        ...
        if as_child_class and retrieve_connections == "skip":
            ...
            for child_class_external_id in as_child_class:
                ...
                items.extend(instantiate_classes(child_cls, [child_cls._to_dict(node) for node in instances.nodes], "retrieve"))
        elif as_child_class:
            ...
            items.extend(instantiate_classes(self._class_type, [self._class_type._to_dict(node) for node in instances.nodes], "retrieve"))
        else:
            for space_key, external_ids in groupby(
                ...
                for ext_id_chunk in chunker(external_id_list, IN_FILTER_CHUNK_SIZE):
                    ...
                    items.extend(instantiate_classes(self._class_type, self._query(filter_, len(ext_id_chunk), retrieve_connections), "retrieve"))
    ...
    def _search(
        ...
        return self._class_list(instantiate_classes(self._class_type, [self._class_type._to_dict(node) for node in nodes], "search"))
    ...
    def _list(self, limit: int, filter: dm.Filter | None, sort: list[InstanceSort] | None = None) -> T_DomainModelList:
        ...
        return self._class_list(instantiate_classes(self._class_type, [self._class_type._to_dict(node) for node in nodes], "list"))
...
class EdgePropertyAPI(EdgeAPI, Generic[T_DomainRelation, T_DomainRelationWrite, T_DomainRelationList], ABC):
    ...
    def _list(  # type: ignore[override]
        ...
        return self._class_list(instantiate_classes(self._class_type, [self._class_type._to_dict(edge) for edge in edges], "list"))
