...
class ConnectionItemF(DomainModel):
    ...
    def as_write(self) -> ConnectionItemFWrite:
        ...
        return ConnectionItemFWrite(
            ...
            direct_list=[direct_list.as_write() if isinstance(direct_list, DomainModel) else direct_list for direct_list in self.direct_list or []],
            name=self.name,
            outwards_multi=[outwards_multi.as_write() for outwards_multi in self.outwards_multi or []],
            outwards_single=self.outwards_single.as_write()
...
class ConnectionItemFWrite(DomainModelWrite):
    ...
    space: str = DEFAULT_INSTANCE_SPACE
    node_type: Union[dm.DirectRelationReference, dm.NodeId, tuple[str, str], None] = dm.DirectRelationReference("pygen-models", "ConnectionItemF")
...
def _create_connection_item_f_filter(
    ...
    if direct_list and isinstance(direct_list, str):
        filters.append(dm.filters.Equals(view_id.as_property_ref("directList"), value={"space": DEFAULT_INSTANCE_SPACE, "externalId": direct_list}))
    if direct_list and isinstance(direct_list, tuple):
        filters.append(dm.filters.Equals(view_id.as_property_ref("directList"), value={"space": direct_list[0], "externalId": direct_list[1]}))
    if direct_list and isinstance(direct_list, list) and isinstance(direct_list[0], str):
        filters.append(dm.filters.In(view_id.as_property_ref("directList"), values=[{"space": DEFAULT_INSTANCE_SPACE, "externalId": item} for item in direct_list]))
    if direct_list and isinstance(direct_list, list) and isinstance(direct_list[0], tuple):
        filters.append(dm.filters.In(view_id.as_property_ref("directList"), values=[{"space": item[0], "externalId": item[1]} for item in direct_list]))
