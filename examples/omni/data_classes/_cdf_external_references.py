...
class _CDFExternalReferencesQuery(NodeQueryCore[T_DomainModelList, CDFExternalReferencesList]):
    ...
    def __init__(
        ...
        self.timeseries = TimeSeriesReferenceAPI(client,  lambda limit: [
            item.timeseries if isinstance(item.timeseries, str) else item.timeseries.external_id or ""
            for item in self._list(limit=limit) if item.timeseries is not None
        ])
