...
if TYPE_CHECKING:
    from omni.data_classes._connection_item_e import ConnectionItemE, ConnectionItemEList, ConnectionItemEGraphQL, ConnectionItemEWrite, ConnectionItemEWriteList
...
class ConnectionItemD(DomainModel):
    ...
    direct_multi: Optional[list[Union[ConnectionItemE, str, dm.NodeId]]] = Field(default=None, repr=False, alias="directMulti")
    direct_single: Union[ConnectionItemE, str, dm.NodeId, None] = Field(default=None, repr=False, alias="directSingle")
    name: Optional[str] = None
    outwards_single: Union[ConnectionItemE, str, dm.NodeId, None] = Field(default=None, repr=False, alias="outwardsSingle")
...
class ConnectionItemDWrite(DomainModelWrite):
    ...
    Args:
        ...
        outwards_single: The outwards single field.
    """
    _container_fields: ClassVar[tuple[str, ...]] = ("direct_multi", "direct_single", "name",)
    _outwards_edges: ClassVar[tuple[tuple[str, dm.DirectRelationReference], ...]] = (("outwards_single", dm.DirectRelationReference("sp_pygen_models", "bidirectionalSingle")),)
    _direct_relations: ClassVar[tuple[str, ...]] = ("direct_multi", "direct_single",)

    _view_id: ClassVar[dm.ViewId] = dm.ViewId("sp_pygen_models", "ConnectionItemD", "1")

    space: str = DEFAULT_INSTANCE_SPACE
    node_type: Union[dm.DirectRelationReference, dm.NodeId, tuple[str, str], None] = dm.DirectRelationReference("sp_pygen_models", "ConnectionItemD")
    direct_multi: Optional[list[Union[ConnectionItemEWrite, str, dm.NodeId]]] = Field(default=None, repr=False, alias="directMulti")
    direct_single: Union[ConnectionItemEWrite, str, dm.NodeId, None] = Field(default=None, repr=False, alias="directSingle")
    name: Optional[str] = None
    outwards_single: Union[ConnectionItemEWrite, str, dm.NodeId, None] = Field(default=None, repr=False, alias="outwardsSingle")
...
class ConnectionItemDList(DomainModelList[ConnectionItemD]):
    ...
    def direct_multi(self) -> ConnectionItemEList:
        ...
        return ConnectionItemEList([item for items in self.data for item in items.direct_multi or [] if isinstance(item, ConnectionItemE)])

    @property
    def direct_single(self) -> ConnectionItemEList:
        from ._connection_item_e import ConnectionItemE, ConnectionItemEList
        return ConnectionItemEList([item.direct_single for item in self.data if isinstance(item.direct_single, ConnectionItemE)])
    @property
    def outwards_single(self) -> ConnectionItemEList:
        from ._connection_item_e import ConnectionItemE, ConnectionItemEList
        return ConnectionItemEList([item.outwards_single for item in self.data if isinstance(item.outwards_single, ConnectionItemE)])
...
class ConnectionItemDWriteList(DomainModelWriteList[ConnectionItemDWrite]):
    ...
    def direct_multi(self) -> ConnectionItemEWriteList:
        ...
        return ConnectionItemEWriteList([item for items in self.data for item in items.direct_multi or [] if isinstance(item, ConnectionItemEWrite)])

    @property
    def direct_single(self) -> ConnectionItemEWriteList:
        from ._connection_item_e import ConnectionItemEWrite, ConnectionItemEWriteList
        return ConnectionItemEWriteList([item.direct_single for item in self.data if isinstance(item.direct_single, ConnectionItemEWrite)])
    @property
    def outwards_single(self) -> ConnectionItemEWriteList:
        from ._connection_item_e import ConnectionItemEWrite, ConnectionItemEWriteList
        return ConnectionItemEWriteList([item.outwards_single for item in self.data if isinstance(item.outwards_single, ConnectionItemEWrite)])


def _create_connection_item_d_filter(
    view_id: dm.ViewId,
    direct_multi: str | tuple[str, str] | dm.NodeId | dm.DirectRelationReference | Sequence[str | tuple[str, str] | dm.NodeId | dm.DirectRelationReference] | None = None,
    direct_single: str | tuple[str, str] | dm.NodeId | dm.DirectRelationReference | Sequence[str | tuple[str, str] | dm.NodeId | dm.DirectRelationReference] | None = None,
    ...
    if isinstance(direct_multi, str | dm.NodeId | dm.DirectRelationReference) or is_tuple_id(direct_multi):
        filters.append(dm.filters.Equals(view_id.as_property_ref("directMulti"), value=as_instance_dict_id(direct_multi)))
    if direct_multi and isinstance(direct_multi, Sequence) and not isinstance(direct_multi, str) and not is_tuple_id(direct_multi):
        filters.append(dm.filters.In(view_id.as_property_ref("directMulti"), values=[as_instance_dict_id(item) for item in direct_multi]))
    if isinstance(direct_single, str | dm.NodeId | dm.DirectRelationReference) or is_tuple_id(direct_single):
        filters.append(dm.filters.Equals(view_id.as_property_ref("directSingle"), value=as_instance_dict_id(direct_single)))
    if direct_single and isinstance(direct_single, Sequence) and not isinstance(direct_single, str) and not is_tuple_id(direct_single):
        filters.append(dm.filters.In(view_id.as_property_ref("directSingle"), values=[as_instance_dict_id(item) for item in direct_single]))
