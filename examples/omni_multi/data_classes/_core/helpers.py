...
if TYPE_CHECKING:
    from omni_multi.data_classes._core.base import DomainModel, T_DomainModel
...
def as_direct_relation_reference(
    ...
    if value is None or isinstance(value, dm.DirectRelationReference):
        ...
        return dm.DirectRelationReference(space=value["space"], external_id=value["external_id"])
    elif hasattr(value, "space") and hasattr(value, "external_id"):
        return dm.DirectRelationReference(space=value.space, external_id=value.external_id)
...
def as_instance_dict_id(value: dm.NodeId | tuple[str, str] | dm.DirectRelationReference | Any) -> dict[str, str]:
