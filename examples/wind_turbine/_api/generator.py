...
class GeneratorAPI(NodeAPI[Generator, GeneratorWrite, GeneratorList, GeneratorWriteList]):
    ...
    def apply(
        ...
        between the nodes. For example, if any of `generator_speed_controller` or `generator_speed_controller_reference` are set, then these
    ...
    def list(
        ...
        Args:
            ...
            limit: Maximum number of generators to return. Defaults to 25. Set to -1, float("inf") or None to return all items.
            filter: (Advanced) If the filtering available in the above is not sufficient, you can write your own filtering which will be ANDed with the filter above.
            retrieve_connections: Whether to retrieve `generator_speed_controller`, `generator_speed_controller_reference` and `nacelle` for the generators. Defaults to 'skip'.
