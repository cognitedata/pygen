...
class GeneratorAPI(NodeAPI[Generator, GeneratorWrite, GeneratorList, GeneratorWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [Generator.model_validate(item) for item in unpacked]
        else:
            retrieved = [Generator.model_construct(**item) for item in unpacked]
        return GeneratorList(retrieved)
