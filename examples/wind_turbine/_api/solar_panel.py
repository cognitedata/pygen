...
class SolarPanelAPI(NodeAPI[SolarPanel, SolarPanelWrite, SolarPanelList, SolarPanelWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [SolarPanel.model_validate(item) for item in unpacked]
        else:
            retrieved = [SolarPanel.model_construct(**item) for item in unpacked]
        return SolarPanelList(retrieved)
