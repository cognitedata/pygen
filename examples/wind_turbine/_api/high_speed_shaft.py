...
class HighSpeedShaftAPI(NodeAPI[HighSpeedShaft, HighSpeedShaftWrite, HighSpeedShaftList, HighSpeedShaftWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [HighSpeedShaft.model_validate(item) for item in unpacked]
        else:
            retrieved = [HighSpeedShaft.model_construct(**item) for item in unpacked]
        return HighSpeedShaftList(retrieved)
