...
class SensorTimeSeriesAPI(
    ...
    def apply(
        ...
        Args:
            sensor_time_series: Sensor time series or
                sequence of sensor time series to upsert.
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [SensorTimeSeries.model_validate(item) for item in unpacked]
        else:
            retrieved = [SensorTimeSeries.model_construct(**item) for item in unpacked]
        return SensorTimeSeriesList(retrieved)
