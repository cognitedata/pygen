...
class WindTurbineAPI(NodeAPI[WindTurbine, WindTurbineWrite, WindTurbineList, WindTurbineWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [WindTurbine.model_validate(item) for item in unpacked]
        else:
            retrieved = [WindTurbine.model_construct(**item) for item in unpacked]
        return WindTurbineList(retrieved)
