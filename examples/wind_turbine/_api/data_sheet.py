...
class DataSheetAPI(NodeAPI[DataSheet, DataSheetWrite, DataSheetList, DataSheetWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [DataSheet.model_validate(item) for item in unpacked]
        else:
            retrieved = [DataSheet.model_construct(**item) for item in unpacked]
        return DataSheetList(retrieved)
