...
class Cognite3DTransformationNodeAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [Cognite3DTransformationNode.model_validate(item) for item in unpacked]
        else:
            retrieved = [Cognite3DTransformationNode.model_construct(**item) for item in unpacked]
        return Cognite3DTransformationNodeList(retrieved)
