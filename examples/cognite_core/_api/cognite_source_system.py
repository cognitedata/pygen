...
class CogniteSourceSystemAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CogniteSourceSystem.model_validate(item) for item in unpacked]
        else:
            retrieved = [CogniteSourceSystem.model_construct(**item) for item in unpacked]
        return CogniteSourceSystemList(retrieved)
