...
class CognitePointCloudVolumeAPI(
    ...
    _properties_by_field = _COGNITEPOINTCLOUDVOLUME_PROPERTIES_BY_FIELD
    ...
    def __call__(
    ...
    ) -> CognitePointCloudVolumeQueryAPI[CognitePointCloudVolumeList]:
        ...
        builder = DataClassQueryBuilder(CognitePointCloudVolumeList)
        return CognitePointCloudVolumeQueryAPI(self._client, builder, filter_, limit)
    ...
    def apply(
        ...
        Note: This method iterates through all nodes and timeseries linked to cognite_point_cloud_volume and creates them including the edges
        between the nodes. For example, if any of `model_3d`, `object_3d` or `revisions` are set, then these
        nodes as well as any nodes linked to them, and all the edges linking these nodes will be created.

        Args:
            cognite_point_cloud_volume: Cognite point cloud volume or sequence of Cognite point cloud volumes to upsert.
    ...
    def list(
        ...
        Args:
            ...
            sort: (Advanced) If sort_by and direction are not sufficient, you can write your own sorting.
                This will override the sort_by and direction. This allowos you to sort by multiple fields and
                specify the direction for each field as well as how to handle null values.
            retrieve_connections: Whether to retrieve `model_3d`, `object_3d` and `revisions` for the Cognite point cloud volumes. Defaults to 'skip'.
