...
class CogniteAssetTypeAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CogniteAssetType.model_validate(item) for item in unpacked]
        else:
            retrieved = [CogniteAssetType.model_construct(**item) for item in unpacked]
        return CogniteAssetTypeList(retrieved)
