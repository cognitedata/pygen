...
class CogniteAssetAPI(NodeAPI[CogniteAsset, CogniteAssetWrite, CogniteAssetList, CogniteAssetWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CogniteAsset.model_validate(item) for item in unpacked]
        else:
            retrieved = [CogniteAsset.model_construct(**item) for item in unpacked]
        return CogniteAssetList(retrieved)
