...
class CogniteActivityAPI(NodeAPI[CogniteActivity, CogniteActivityWrite, CogniteActivityList, CogniteActivityWriteList]):
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CogniteActivity.model_validate(item) for item in unpacked]
        else:
            retrieved = [CogniteActivity.model_construct(**item) for item in unpacked]
        return CogniteActivityList(retrieved)
