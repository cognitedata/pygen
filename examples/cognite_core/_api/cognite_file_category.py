...
class CogniteFileCategoryAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CogniteFileCategory.model_validate(item) for item in unpacked]
        else:
            retrieved = [CogniteFileCategory.model_construct(**item) for item in unpacked]
        return CogniteFileCategoryList(retrieved)
