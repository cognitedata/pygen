...
class CogniteCADRevisionAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [CogniteCADRevision.model_validate(item) for item in unpacked]
        else:
            retrieved = [CogniteCADRevision.model_construct(**item) for item in unpacked]
        return CogniteCADRevisionList(retrieved)
