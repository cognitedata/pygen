...
class Cognite360ImageCollectionAPI(
    ...
    def _query(
        ...
        if global_config.validate_retrieve:
            retrieved = [Cognite360ImageCollection.model_validate(item) for item in unpacked]
        else:
            retrieved = [Cognite360ImageCollection.model_construct(**item) for item in unpacked]
        return Cognite360ImageCollectionList(retrieved)
