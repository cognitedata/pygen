...
class MovieActorsAPI:
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> dm.EdgeList:
        f = dm.filters
        is_edge_type = f.Equals(["edge", "type"], {"space": "IntegrationTestsImmutable", "externalId": "Movie.actors"})
...
class MovieDirectorsAPI:
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> dm.EdgeList:
        ...
        is_edge_type = f.Equals(
            ["edge", "type"], {"space": "IntegrationTestsImmutable", "externalId": "Movie.directors"}
        )
        if isinstance(external_id, str):
            is_movie = f.Equals(
                ["edge", "startNode"], {"space": "IntegrationTestsImmutable", "externalId": external_id}
...
class MoviesAPI(TypeAPI[Movie, MovieApply, MovieList]):
    ...
    def apply(self, movie: MovieApply, replace: bool = False) -> dm.InstancesApplyResult:
        return self._client.data_modeling.instances.apply(nodes=movie.to_instances_apply(), replace=replace)
