...
from movie_domain.client.data_classes.roles import Role, RoleApply, RoleList
...
class RoleMoviesAPI:
    ...
    def retrieve(self, external_id: str | Sequence[str]) -> dm.EdgeList:
        ...
        if isinstance(external_id, str):
            is_role = f.Equals(["edge", "startNode"], {"space": "IntegrationTestsImmutable", "externalId": external_id})
...
class RolesAPI(TypeAPI[Role, RoleApply, RoleList]):
    ...
    def apply(self, role: RoleApply, replace: bool = False) -> dm.InstancesApplyResult:
        return self._client.data_modeling.instances.apply(nodes=role.to_node(), replace=replace)
