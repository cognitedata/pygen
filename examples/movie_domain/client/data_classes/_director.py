...
class Director(DomainModel):
    """This represents the reading version of director.
    ...
    type: dm.DirectRelationReference = dm.DirectRelationReference("IntegrationTestsImmutable", "RoleDirector")
...
class DirectorApply(DomainModelApply):
    """This represents the writing version of director.
...
class DirectorList(DomainModelList[Director]):
    """List of directors in the read version."""

    _INSTANCE = Director

    def as_apply(self) -> DirectorApplyList:
        """Convert these read versions of director to the writing versions."""
        return DirectorApplyList([node.as_apply() for node in self.data])


class DirectorApplyList(DomainModelApplyList[DirectorApply]):
    """List of directors in the writing version."""

    _INSTANCE = DirectorApply


def _create_director_filter(
    view_id: dm.ViewId,
    ...
    if won_oscar is not None and isinstance(won_oscar, bool):
