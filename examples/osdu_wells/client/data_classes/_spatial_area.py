...
class SpatialArea(DomainModel):
    """This represents the reading version of spatial area.

    It is used to when data is retrieved from CDF.

    Args:
        space: The space where the node is located.
        external_id: The external id of the spatial area.
        ...
        created_time: The created time of the spatial area node.
        last_updated_time: The last updated time of the spatial area node.
        deleted_time: If present, the deleted time of the spatial area node.
        version: The version of the spatial area node.
    """
    ...
    def as_apply(self) -> SpatialAreaApply:
        """Convert this read version of spatial area to the writing version."""
...
class SpatialAreaApply(DomainModelApply):
    """This represents the writing version of spatial area.
    ...
    Args:
        ...
        spatial_parameter_type_id: The spatial parameter type id field.
        wgs_84_coordinates: The wgs 84 coordinate field.
        existing_version: Fail the ingestion request if the spatial area version is greater than or equal to this value.
    ...
    def _to_instances_apply(
        ...
        write_view = (view_by_write_class and view_by_write_class.get(type(self))) or dm.ViewId(
            "IntegrationTestsImmutable", "SpatialArea", "312323f14f3d3f"
        )
...
class SpatialAreaList(DomainModelList[SpatialArea]):
    """List of spatial areas in the read version."""

    _INSTANCE = SpatialArea

    def as_apply(self) -> SpatialAreaApplyList:
        """Convert these read versions of spatial area to the writing versions."""
        return SpatialAreaApplyList([node.as_apply() for node in self.data])


class SpatialAreaApplyList(DomainModelApplyList[SpatialAreaApply]):
    """List of spatial areas in the writing version."""

    _INSTANCE = SpatialAreaApply


def _create_spatial_area_filter(
    view_id: dm.ViewId,
