...
class UnacceptableUsage(DomainModel):
    """This represents the reading version of unacceptable usage.
    ...
    def as_apply(self) -> UnacceptableUsageApply:
        """Convert this read version of unacceptable usage to the writing version."""
...
class UnacceptableUsageApply(DomainModelApply):
    """This represents the writing version of unacceptable usage.
    ...
    Args:
        ...
        workflow_persona_type_id: The workflow persona type id field.
        workflow_usage_type_id: The workflow usage type id field.
        existing_version: Fail the ingestion request if the unacceptable usage version is greater than or equal to this value.
...
class UnacceptableUsageList(DomainModelList[UnacceptableUsage]):
    """List of unacceptable usages in the read version."""

    _INSTANCE = UnacceptableUsage

    def as_apply(self) -> UnacceptableUsageApplyList:
        """Convert these read versions of unacceptable usage to the writing versions."""
        return UnacceptableUsageApplyList([node.as_apply() for node in self.data])


class UnacceptableUsageApplyList(DomainModelApplyList[UnacceptableUsageApply]):
    """List of unacceptable usages in the writing version."""

    _INSTANCE = UnacceptableUsageApply


def _create_unacceptable_usage_filter(
    view_id: dm.ViewId,
