...
class SpatialAreaQueryAPI(QueryAPI[T_DomainModelList]):
    def __init__(
        ...
        self._builder.append(
            QueryStep(
                ...
                select=dm.query.Select([dm.query.SourceSelector(self._view_by_write_class[SpatialAreaApply], ["*"])]),
    ...
    def query(
        ...
        Args:
            retrieve_as_ingested_coordinates: Whether to retrieve the as ingested coordinate for each spatial area or not.
            retrieve_wgs_84_coordinates: Whether to retrieve the wgs 84 coordinate for each spatial area or not.
        ...
        self._builder.append(
            QueryStep(
                ...
                expression=dm.query.NodeResultSetExpression(
                    ...
                    from_=from_,
                    through=self._view_by_write_class[SpatialAreaApply].as_property_ref("as_ingested_coordinates"),
                    direction="outwards",
                ),
        ...
        self._builder.append(
            QueryStep(
                ...
                expression=dm.query.NodeResultSetExpression(
                    filter=dm.filters.HasData(views=[view_id]),
                    from_=from_,
                    through=self._view_by_write_class[SpatialAreaApply].as_property_ref("wgs_84_coordinates"),
                    direction="outwards",
                ),
                select=dm.query.Select([dm.query.SourceSelector(view_id, ["*"])]),
                max_retrieve_limit=-1,
