...
from field.client.config import global_config
...
_GRAPHQL_DATA_CLASS_BY_DATA_MODEL_BY_TYPE: dict[dm.DataModelId, dict[str, type[GraphQLCore]]] = {
    dm.DataModelId("fields-space", "Fields", "4"): {
        "Field": data_classes.FieldGraphQL,
        "Area": data_classes.AreaGraphQL,
        "Region": data_classes.RegionGraphQL,
    },
}
...
class PygenValidationError(ValueError):
    ...
    def __str__(self):
        return (
            ...
            f" importing `from field.client.config import global_config`."
        )
